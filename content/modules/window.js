        ...
        () => {
          Splash.show(
            DOM(
              ...
              DOM(
                { style: 'splash-help-container' },
                DOM( 
                  {
                    tag: 'img',
                    style: 'splash-help-img',
                    src: 'content/img/shop/help-1.png',
                  }
                ),
                DOM(
                  { style: 'splash-help-paragraph' },
                  
                ).innerHTML = 'На <span>«Ярмарке»</span> представлен ассортимент товаров, распределённый по категориям: скины, флаги, рамки.',

                DOM(
                  { style: 'splash-help-paragraph' },
                  'Ассортимент обновляется каждые 24 часа (00:00 мск). Стоимость формируется автоматически. Уже купленный товар может снова появиться после обновления.',
                ),

                DOM(
                  { style: 'splash-help-paragraph' },
                  'Чтобы приобрести товар, нажмите ЛКМ по иконке и подтвердите покупку. Вернуть товар невозможно. Один и тот же товар нельзя купить дважды.',
                ),

                DOM({ style: 'splash-help-paragraph' }, 'Все приобретённые товары перемещаются во вкладку «Коллекции».'),

                DOM(
                  { style: 'splash-help-paragraph' },
                  'В дальнейшем планируются обновления магазина как по ассортименту, так и по механике «Ярмарки».',
                ),

                DOM({ style: 'splash-help-paragraph' }, '• Скины — доступны всегда.'),
                DOM({ style: 'splash-help-paragraph' }, '• Флаги — обновляются полностью по таймеру.'),
                DOM(
                  { style: 'splash-help-paragraph' },
                  '• Рамки — делятся на обычные и сезонные. Сезон длится 3 месяца. Товар доступен только в этот период, затем заменяется новым.',
                ),

                DOM(
                  { style: 'splash-help-paragraph' },
                  'Раздел «Коллекции» содержит все предметы, полученные на «Ярмарке» или через достижения.',
                ),

                DOM(
                  { style: 'splash-help-paragraph' },
                  'Чтобы изменить флаг или рамку на аккаунте, нажмите на нужный предмет и выберите «Выбрать».',
                ),

                DOM(
                  { style: 'splash-help-paragraph' },
                  'Купленный скин автоматически становится доступным в списке образов героя через кнопку «Скины» в билде.',
                ),
              ),

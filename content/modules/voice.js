...
export class Voice {
	...
	static async initLocalMedia() {
		...
		Voice.startIndication();
	}

	static startIndication() {
		if (!Voice.mic || !Voice.userMedia) {
			return;
		}
		if (Voice.intervalId) clearInterval(Voice.intervalId);
		Voice.intervalId = Voice.indication(Voice.userMedia, (percent) => {
			let levelBar = Voice.infoPanel.querySelector('.voice-info-panel-body-item-bar-level');
			if (levelBar) {
			...
			if (NativeAPI.overlayProcess) {
				try {
					let data = {
						html: Voice.infoPanel ? Voice.infoPanel.innerHTML : '',
						level: Voice.mic.enabled ? percent : 0
					};
					if (Voice.lastLevel !== data.level) {
						NativeAPI.sendToClients(data);
						Voice.lastLevel = data.level;
					}
				} catch (e) {
				}
			}
	...
	static async toggleEnabledMic() {
		...
		if (Voice.mic.enabled) {
			...
			if (Voice.infoPanel && Voice.infoPanel.firstChild && Voice.infoPanel.firstChild.lastChild) {
